<?php
/**
 * Blog Management - Create View
 * Create new blog post with word counter and visual indicators
 */
?>

<div class="bg-white shadow rounded-lg">
    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        <h2 class="text-lg font-semibold text-gray-900">Create Blog Post</h2>
    ...
    <div class="p-6">
        ...
        <form method="POST" action="/admin/blog/store" enctype="multipart/form-data" id="blogForm">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" required maxlength="255" value="<?= $this->escape($form_data['title'] ?? '') ?>" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out sm:text-sm <?= isset($form_errors['title']) ? 'border-red-300 bg-red-50' : 'bg-white hover:border-gray-400' ?>" placeholder="Enter blog post title...">
                ...
                <div>
                    ...
                    <input type="text" name="url_alias" id="url_alias" required maxlength="255" value="<?= $this->escape($form_data['url_alias'] ?? '') ?>" pattern="[a-z0-9\-]+" title="Only lowercase letters, numbers, and hyphens allowed" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out sm:text-sm <?= isset($form_errors['url_alias']) ? 'border-red-300 bg-red-50' : 'bg-white hover:border-gray-400' ?>" placeholder="e.g., my-blog-post-title">
                ...
                <div>
                    ...
                    <input type="text" name="tags" id="tags" maxlength="500" value="<?= $this->escape($form_data['tags'] ?? '') ?>" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out sm:text-sm bg-white hover:border-gray-400" placeholder="e.g., photography, tips, tutorial">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Upload an image for the blog post (max 5MB).</p>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-2 space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-3">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="related_content[]" value="<?= $content['content_id'] ?>" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="meta_keywords" id="meta_keywords" value="<?= $this->escape($form_data['meta_keywords'] ?? '') ?>" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out sm:text-sm bg-white hover:border-gray-400" placeholder="comma, separated, keywords">
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit" name="action" value="draft" class="px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Save as Draft
                    </button>
                    <button type="submit" name="action" value="publish" class="px-4 py-2 bg-blue-600 border border-transparent text-sm font-medium rounded-md text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Publish Blog Post
                    </button>
                </div>
            </div>
...
<script>
// Word counter functionality with visual indicators
document.addEventListener('DOMContentLoaded', function() {
    ...
    titleField.addEventListener('input', function() {
        if (urlAliasField.value === '' || urlAliasField.dataset.autoGenerated === 'true') {
            const alias = titleField.value
                .toLowerCase()
                .replace(/[^a-z0-9\s-]/g, '')
                .replace(/\s+/g, '-')
                .replace(/-+/g, '-')
            ...
            urlAliasField.dataset.autoGenerated = 'true';
        }
    });
    
    urlAliasField.addEventListener('input', function() {
        if (urlAliasField.value !== '') {
            urlAliasField.dataset.autoGenerated = 'false';
        }
    });
    ...
    document.getElementById('blogForm').addEventListener('submit', function(e) {
        ...
        if (actionButton && actionButton.name === 'action') {
            ...
            statusField.value = actionButton.value === 'publish' ? 'published' : 'draft';
        }
    });
});
</script>