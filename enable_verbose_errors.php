<?php
// Enable verbose PHP error reporting and debug the redirect issue
echo "=== ENABLING VERBOSE ERROR REPORTING ===\n\n";

// Create a comprehensive error enabling script
$verboseErrors = '<?php
...
file_put_contents('enable_errors.php', $verboseErrors);
echo "1. Created error enabling script\n";

// Push to GitHub
exec('git add enable_errors.php && git commit -m "Enable verbose PHP error reporting for debugging" && git push origin main 2>&1', $output);
echo "2. Pushed to GitHub\n";

// Since we can't pull normally, let's check the error log
echo "\n3. Checking current error log...\n";
$agentUrl = 'https://dalthaus.net/agent.php';
$key = 'dalthaus_agent_key_2025';
$errorLogUrl = $agentUrl . '?action=error_log&key=' . $key . '&lines=20';
$response = @file_get_contents($errorLogUrl);
if ($response) {
