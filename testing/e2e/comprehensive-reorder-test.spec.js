...
test.describe('Comprehensive Reorder Functionality', () => {
    ...
    test('6. Dashboard Reorder Button Link', async () => {
        console.log('\n[TEST 6] Testing dashboard reorder button...');

        await page.goto('https://dalthaus.net/admin/dashboard', { waitUntil: 'networkidle' });

        // Find reorder button
        const reorderButton = page.locator('a:has-text("Reorder Content")');
        await expect(reorderButton).toBeVisible();
        console.log('[TEST 6] Reorder button found on dashboard');

        // Check href
        const href = await reorderButton.getAttribute('href');
        expect(href).toBe('/admin/content/reorder');
        console.log(`[TEST 6] Button links to: ${href}`);

        // Click and verify navigation
        const errorsBefore = consoleErrors.length;
        await reorderButton.click();
        await page.waitForURL('**/admin/content/reorder');

        await expect(page.locator('h2')).toContainText('Reorder Content');
        console.log('[TEST 6] Navigation successful');
    ...
    test('7. Content Page Reorder Button Link', async () => {
        console.log('\n[TEST 7] Testing content page reorder button...');

        await page.goto('https://dalthaus.net/admin/content', { waitUntil: 'networkidle' });

        // Find reorder button
        const reorderButton = page.locator('a:has-text("Reorder")').first();
        await expect(reorderButton).toBeVisible();
        console.log('[TEST 7] Reorder button found on content page');

        // Check href
        const href = await reorderButton.getAttribute('href');
        expect(href).toBe('/admin/content/reorder');
        console.log(`[TEST 7] Button links to: ${href}`);
        ...
        await expect(page.locator('h2')).toContainText('Reorder Content');
        console.log('[TEST 7] Navigation successful');
